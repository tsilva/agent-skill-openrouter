#!/usr/bin/env python3
"""
Claude Code Settings Cleaner

Analyzes and cleans up permission whitelists in Claude Code settings files.
Identifies dangerous patterns, overly specific approvals, and redundant entries.
"""
...
class SettingsCleaner:
    """Analyzes and cleans Claude Code permission settings"""

    # Dangerous patterns that grant overly broad access
    DANGEROUS_PATTERNS = {
        "Bash(*:*)",
        "Read(/*)",
        "Write(/*)",
    ...
    def is_overly_specific(self, pattern: str) -> Tuple[bool, Optional[str]]:
        ...
        Returns (is_specific, suggested_generalization)
        """
        ...
        args = pattern[5:-1]  # Remove "Bash(" and ")"

        # If it has :* it's already a wildcard pattern
        if ":*" in args or args == "*:*":
            return False, None

        # If it contains a space, it has arguments (overly specific)
        if " " in args:
    ...
    def is_pattern_subset(self, specific: str, general: str) -> bool:
        ...
        # If specific has no args but general does, not a subset
        if '(' not in specific:
            return False
        ...
        if gen_args.endswith(":*"):
            base_cmd = gen_args[:-2]
            # Check if specific starts with the base command
            if spec_args.startswith(base_cmd):
                # Handle exact match or command with arguments
    ...
    def is_redundant(self, perm: Permission) -> Optional[Permission]:
        ...
        Returns the covering permission if redundant, None otherwise.
        """
        # Only project permissions can be redundant
        if perm.location != "Project":
            return None

        # Check against all global permissions
            ...
            if self.is_dangerous(pattern):
                ...
                if covered_by:
                    self.issues.append(Issue(
                        perm,
                        IssueType.REDUNDANT,
                        f"Covered by global permission",
    ...
    def print_report(self, grouped: Dict[IssueType, List[Issue]]):
        ...
        # Summary
        total_issues = len(dangerous) + len(specific) + len(redundant)
        if total_issues == 0:
    ...
    def save_settings(self, global_perms: Set[str], project_perms: Set[str]) -> bool:
    ...
    def interactive_clean(self, grouped: Dict[IssueType, List[Issue]]):
        ...
        if total_changes > 0:
            ...
            if self.save_settings(global_perms, project_perms):
    ...
    def auto_fix(self, grouped: Dict[IssueType, List[Issue]]):
        """Automatically remove redundant permissions only (safest option)"""
        redundant = grouped[IssueType.REDUNDANT]

        if not redundant:
            print(f"\n{Fore.GREEN}No redundant permissions found.{Style.RESET_ALL}")
            return

        print(f"\n{Fore.BLUE}{Style.BRIGHT}Auto-fixing {len(redundant)} redundant permission(s)...{Style.RESET_ALL}\n")
        ...
        if self.save_settings(self.global_permissions, project_perms):
            print(f"\n{Fore.GREEN}{Style.BRIGHT}✓ Removed {len(redundant)} redundant permission(s){Style.RESET_ALL}\n")
        else:
            print(f"\n{Fore.RED}Failed to save changes.{Style.RESET_ALL}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze and clean up Claude Code permission whitelists"
    )
    parser.add_argument(
        "command",
    ...
    # Initialize cleaner
    cleaner = SettingsCleaner(args.global_settings, args.project_settings)

    # Load settings
    if not cleaner.load_settings():
        return 1

    # Analyze permissions
    grouped = cleaner.analyze()

    # Execute command
    if args.command == "analyze":
        cleaner.print_report(grouped)
    elif args.command == "clean":
        cleaner.print_report(grouped)
        cleaner.interactive_clean(grouped)
    elif args.command == "auto-fix":
        cleaner.auto_fix(grouped)
